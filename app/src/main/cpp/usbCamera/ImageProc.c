...
void pixelToBmp(JNIEnv* env, int index, jobject bitmap){
	...
	colors = (int*)pixels;
	int *lrgb = NULL;
	lrgb = &pUsbCamera[index]->rgbBuffer[0];
 ...
 * @return
 */
void initCameraCount(int count) {
    UsbCamera usbCamera[count];
    for(int i=0; i<count; i++) {
        pUsbCamera[i] = &usbCamera[i];
    }
}

int prepareCameraWithBase(int index, int width, int height, int pixFormat){
	int ret;

	pUsbCamera[index] = (UsbCamera *) malloc(sizeof(UsbCamera));
	memset(pUsbCamera[index],'\0', sizeof(UsbCamera));
	UsbCamera *pCamera = pUsbCamera[index];
	sprintf(pCamera->deviceName, "/dev/video%d", index);
...
int processCamera(int index){
	int ret = 0;
	ret = readFrameOnce(pUsbCamera[index]);
	yuyv422toABGRY(pUsbCamera[index]->rgbBuffer, pUsbCamera[index]->framePreviewBuffer, pUsbCamera[index]->width, pUsbCamera[index]->height);
...
void stopCamera(JNIEnv *env, int index){
	if (pUsbCamera[index]->isRecording) {
		stopRecord(env, index);
	}
	stopCapturing (pUsbCamera[index]);
	uninitDevice (pUsbCamera[index]);
	closeDevice (pUsbCamera[index]);
	SAFE_FREE_ELEMENT(pUsbCamera[index]);
}

void *videoRecordThread(void *pVal, int index) {
...
int startRecord(JNIEnv *env, jobject thiz, int index){
	...
	pUsbCamera[index]->isRecording = 1;
	int ret = pthread_create(&record_id, NULL, videoRecordThread, NULL);
	return ret;
}

int stopRecord(JNIEnv *env, int index){
	pUsbCamera[index]->isRecording = 0;
